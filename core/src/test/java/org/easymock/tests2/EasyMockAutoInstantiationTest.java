...
public class EasyMockAutoInstantiationTest extends EasyMockSupport {
    ...
    public static class ToInjectWithOnlyOneConstructor {
        protected IMethods m1;
        ...
        private ToInjectWithOnlyOneConstructor(int number) {
        }
    }

    private static class OnlyOneConstructorInjectionTest {
        @Mock
        protected IMethods m;
        ...
        @TestSubject
        protected ToInjectWithOnlyOneConstructor toInject;
    }

    @Test
    public void shouldInjectMocksToTargetSubjectWithOnlyOneConstructor() {
        OnlyOneConstructorInjectionTest test = new OnlyOneConstructorInjectionTest();
        EasyMockSupport.injectMocks(test);
        assertSame(test.m, test.toInject.m1);
        assertSame(test.m, test.toInject.m2);
        assertSame(test.v, test.toInject.v);
        assertNull(test.toInject.a);
        assertNull(test.toInject.f);
        assertNull(ToInjectWithOnlyOneConstructor.s);
    }

    @Test
    public void dummyToInstantiateTestSubjects() {
        new ToInjectWithOnlyOneConstructor(1);
    }
}
